...
#define POINTS_LAB4_STACK_ADD			15
#define POINTS_LAB4_QUEUE_REMOVE		10
#define POINTS_LAB4_STACK_REMOVE		10
#define POINTS_LAB4_INSERT_ITER			15
#define POINTS_LAB4_INSERT_INDEX		10
#define POINTS_LAB4_REMOVE_DECIMAL		25
...
#ifndef _GRADING
	results::WriteResults("Lab4.h", "Lab4.results");
#endif
	int totalScore = 0;

#if LAB4_QUEUE_ADD
	totalScore += Battery_AddQueueOrdering(); 
#endif
#if LAB4_STACK_ADD
	totalScore += Battery_AddStackOrdering();
#endif
#if LAB4_QUEUE_REMOVE
	totalScore += Battery_RemoveQueueOrdering();
#endif
#if LAB4_STACK_REMOVE
	totalScore += Battery_RemoveStackOrdering();
#endif
#if LAB4_INSERT_ITER
	totalScore += Battery_InsertWithIterator();
#endif
#if LAB4_INSERT_INDEX
	totalScore += Battery_InsertWithIndex();
#endif
#if LAB4_REMOVE_DECIMAL
	totalScore += Battery_RemoveDecimalGreater();
#endif
...
#if LAB4_QUEUE_ADD
int UnitTests_Lab4::Battery_AddQueueOrdering() {
	FailVector failVec;
	...
	int totalDeductions = UnitTestBattery("Testing adding to a list using queue ordering", POINTS_LAB4_QUEUE_ADD,failVec, passVec);
	int scoreForTest = CalculatePointsForTest(POINTS_LAB4_QUEUE_ADD, totalDeductions);
...
#if LAB4_STACK_ADD
int UnitTests_Lab4::Battery_AddStackOrdering() {
	FailVector failVec;
	...
	int totalDeductions = UnitTestBattery("Testing adding to a list using stack ordering", POINTS_LAB4_STACK_ADD,failVec, passVec);
	int scoreForTest = CalculatePointsForTest(POINTS_LAB4_STACK_ADD, totalDeductions);
...
#if LAB4_QUEUE_REMOVE
int UnitTests_Lab4::Battery_RemoveQueueOrdering() {
	FailVector failVec;
	...
	int totalDeductions = UnitTestBattery("Testing removing from a list using queue ordering", POINTS_LAB4_QUEUE_REMOVE,failVec, passVec);
	int scoreForTest = CalculatePointsForTest(POINTS_LAB4_QUEUE_REMOVE, totalDeductions);
...
#if LAB4_STACK_REMOVE
int UnitTests_Lab4::Battery_RemoveStackOrdering() {
	FailVector failVec;
	...
	int totalDeductions = UnitTestBattery("Testing removing from a list using stack ordering", POINTS_LAB4_STACK_REMOVE,failVec, passVec);
	int scoreForTest = CalculatePointsForTest(POINTS_LAB4_STACK_REMOVE, totalDeductions);
...
#if LAB4_INSERT_ITER
int UnitTests_Lab4::Battery_InsertWithIterator() {
	FailVector failVec;
	...
	int totalDeductions = UnitTestBattery("Testing inserting into a list using an iterator", POINTS_LAB4_INSERT_ITER,failVec, passVec);
	int scoreForTest = CalculatePointsForTest(POINTS_LAB4_INSERT_ITER, totalDeductions);
...
#if LAB4_INSERT_INDEX
int UnitTests_Lab4::Battery_InsertWithIndex() {
	FailVector failVec;
	// failVec.push_back(InsertWithIndex_IfWasUsed);	// Commented out to catch plagiarism
	...
	int totalDeductions = UnitTestBattery("Testing inserting into a list at a specified index", POINTS_LAB4_INSERT_INDEX,failVec, passVec);
	int scoreForTest = CalculatePointsForTest(POINTS_LAB4_INSERT_INDEX, totalDeductions);

	return scoreForTest;
}

#pragma region Fail Tests
FailResult UnitTests_Lab4::InsertWithIndex_IfWasUsed() {
	bool usesConditional = SearchFile("Lab4.h", "void Insert(int", "void Insert(std::", "if(");

	FailResult result;

	result.check = usesConditional == true;
	result.deduction = 15;
	result.msg = "This method should not use a conditional statement";
...
#if LAB4_REMOVE_DECIMAL
int UnitTests_Lab4::Battery_RemoveDecimalGreater() {
	FailVector failVec;
	...
	int totalDeductions = UnitTestBattery("Testing removing values from list matching criteria", POINTS_LAB4_REMOVE_DECIMAL,failVec, passVec);
	int scoreForTest = CalculatePointsForTest(POINTS_LAB4_REMOVE_DECIMAL, totalDeductions);
...
FailResult UnitTests_Lab4::Fail_RemoveDecimalGreaterFixed_IncorrectValuesRemoved() {
	...
	FailResult result;
	result.check = fixedList.mList.size() && *fixedList.mList.begin() != randomDec;
	result.deduction = 25;
	result.msg = "Not all values matching criteria were removed (fixed list)";
...
FailResult UnitTests_Lab4::Fail_RemoveDecimalGreaterRandom_IncorrectNumberOfValuesAreRemoved() {
	...
	result.deduction = 20;
	result.msg = "Incorrect number of values were removed (random list)";
