...
#define POINTS_HUFFMAN_CTOR					5
#define POINTS_HUFFMAN_GENERATE_FREQUENCY	10
#define POINTS_HUFFMAN_GENERATE_LEAFLIST	10
#define POINTS_HUFFMAN_GENERATE_TREE		15
#define POINTS_HUFFMAN_CLEAR_TREE			3
#define POINTS_HUFFMAN_DTOR					2
#define POINTS_HUFFMAN_GENERATE_ENCODING	15
#define POINTS_HUFFMAN_COMPRESS				20
#define POINTS_HUFFMAN_DECOMPRESS			20
...
#ifndef _GRADING
	results::WriteResults("Huffman.h", "Huffman.results");
#endif
	int totalScore = 0;

#if HUFFMAN_CTOR
	totalScore += Battery_Constructor();
#endif
#if HUFFMAN_GENERATE_FREQUENCY
	totalScore += Battery_GenerateFrequencyTable();
#endif
#if HUFFMAN_GENERATE_LEAFLIST
	totalScore += Battery_GenerateLeafList();
#endif
#if HUFFMAN_GENERATE_TREE
	totalScore += Battery_GenerateTree();
#endif
#if HUFFMAN_CLEAR_TREE
	totalScore += Battery_ClearTree();
#endif
#if HUFFMAN_DTOR
	totalScore += Battery_Destructor();
#endif
#if HUFFMAN_GENERATE_ENCODING
	totalScore += Battery_GenerateEncodingTable();
#endif
#if HUFFMAN_COMPRESS
	totalScore += Battery_Compress();
#endif
#if HUFFMAN_DECOMPRESS
	totalScore += Battery_Decompress();
#endif
...
#if HUFFMAN_CTOR
int UnitTests_Lab8::Battery_Constructor() {
	FailVector failVec;
	...
	int totalDeductions = UnitTestBattery("Testing Huffman constructor", POINTS_HUFFMAN_CTOR,failVec, passVec);
	int scoreForTest = CalculatePointsForTest(POINTS_HUFFMAN_CTOR, totalDeductions);
...
#if HUFFMAN_GENERATE_FREQUENCY
int UnitTests_Lab8::Battery_GenerateFrequencyTable() {
	GenerateRandomFile();
	...
	int totalDeductions = UnitTestBattery("Testing GenerateFrequencyTable", POINTS_HUFFMAN_GENERATE_FREQUENCY,failVec, passVec);
	int scoreForTest = CalculatePointsForTest(POINTS_HUFFMAN_GENERATE_FREQUENCY, totalDeductions);
...
#if HUFFMAN_GENERATE_LEAFLIST
int UnitTests_Lab8::Battery_GenerateLeafList() {
	FailVector failVec;
	...
	int totalDeductions = UnitTestBattery("Testing GenerateLeafList", POINTS_HUFFMAN_GENERATE_LEAFLIST,failVec, passVec);
	int scoreForTest = CalculatePointsForTest(POINTS_HUFFMAN_GENERATE_LEAFLIST, totalDeductions);
...
int UnitTests_Lab8::Battery_GenerateTree() {
	FailVector failVec;
	...
	int totalDeductions = UnitTestBattery("Testing GenerateTree", POINTS_HUFFMAN_GENERATE_TREE,failVec, passVec);
	int scoreForTest = CalculatePointsForTest(POINTS_HUFFMAN_GENERATE_TREE, totalDeductions);
...
#if HUFFMAN_CLEAR_TREE
int UnitTests_Lab8::Battery_ClearTree() {
	FailVector failVec;
	...
	int totalDeductions = UnitTestBattery("Testing ClearTree", POINTS_HUFFMAN_CLEAR_TREE,failVec, passVec);
	int scoreForTest = CalculatePointsForTest(POINTS_HUFFMAN_CLEAR_TREE, totalDeductions);
...
#if HUFFMAN_DTOR
int UnitTests_Lab8::Battery_Destructor() {
	FailVector failVec;
	...
	int totalDeductions = UnitTestBattery("Testing destructor", POINTS_HUFFMAN_DTOR,failVec, passVec);
	int scoreForTest = CalculatePointsForTest(POINTS_HUFFMAN_DTOR, totalDeductions);
...
#if HUFFMAN_GENERATE_ENCODING
int UnitTests_Lab8::Battery_GenerateEncodingTable() {
	FailVector failVec;
	failVec.push_back(Fail_GenerateEncodingTable_UsesArrowOperatorOnEncodingTable);
	//failVec.push_back(Fail_GenerateEncodingTable_DoesNotUseValue);
	...
	int totalDeductions = UnitTestBattery("Testing GenerateEncodingTable", POINTS_HUFFMAN_GENERATE_ENCODING,failVec, passVec);
	int scoreForTest = CalculatePointsForTest(POINTS_HUFFMAN_GENERATE_ENCODING, totalDeductions);
...
FailResult UnitTests_Lab8::Fail_GenerateEncodingTable_DoesNotUseValue() {
	bool usesValue = SearchFile("Huffman.h", "void GenerateEncodingTable(", "void Compress(", "->value]");

	FailResult result;
	result.check = usesValue == false;
	result.deduction = 5;
	result.msg = "Not working with correct index of the encoding table";
...
FailResult UnitTests_Lab8::Fail_GenerateEncodingTable_DataAddedToIncorrectIndices() {
	...
	// Test data
	std::vector<bool> encodingData[9];
...
FailResult UnitTests_Lab8::Fail_GenerateEncodingTable_DataIsNotReversed() {
	...
	FailResult result;
	result.check = correctEncoding == false;
	result.deduction = 15;
...
#if HUFFMAN_COMPRESS
int UnitTests_Lab8::Battery_Compress() {
	...
	failVec.push_back(Fail_Compress_DoesNotPassCorrectSizeForHeader);
	...
	int totalDeductions = UnitTestBattery("Testing Compress", POINTS_HUFFMAN_COMPRESS,failVec, passVec);
	int scoreForTest = CalculatePointsForTest(POINTS_HUFFMAN_COMPRESS, totalDeductions);
...
FailResult UnitTests_Lab8::Fail_Compress_DoesNotPassHeader() {
	bool passesFreqTable = SearchFile("Huffman.h", "void Compress(", "void Decompress(", ",(const char*)mFrequencyTable") ||
		SearchFile("Huffman.h", "void Compress(", "void Decompress(", ",(char*)mFrequencyTable") ||
		SearchFile("Huffman.h", "void Compress(", "void Decompress(", ",reinterpret_cast<const char*>(mFrequencyTable)");
...
FailResult UnitTests_Lab8::Fail_Compress_DoesNotPassCorrectSizeForHeader() {
	bool usesCorrectHeaderSize = SearchFile("Huffman.h", "void Compress(", "void Decompress(", ",1024") ||
		SearchFile("Huffman.h", "void Compress(", "void Decompress(", ",sizeof(int)*256") ||
		SearchFile("Huffman.h", "void Compress(", "void Decompress(", ",256*sizeof(int)") ||
		SearchFile("Huffman.h", "void Compress(", "void Decompress(", ",sizeof(unsigned int)*256") ||
		SearchFile("Huffman.h", "void Compress(", "void Decompress(", ",256*sizeof(unsigned int)") ||
		SearchFile("Huffman.h", "void Compress(", "void Decompress(", ",256*4)") ||
		SearchFile("Huffman.h", "void Compress(", "void Decompress(", ",4*256)")
		;

	FailResult result;
	result.check = usesCorrectHeaderSize == false;
	result.deduction = 5;
	result.msg = "Does not pass correct size of Huffman header. Third argument should be the number of bytes of the frequency table";
...
FailResult UnitTests_Lab8::Fail_Compress_OpensIncorrectFileForReading() {
	bool opensInputFileWithIfstream = SearchFile("Huffman.h", "void Compress(", "void Decompress(", "mFileName, std::ios::binary") ||
		SearchFile("Huffman.h", "void Compress(", "void Decompress(", "mFileName, std::ios_base::binary") ||
		SearchFile("Huffman.h", "void Compress(", "void Decompress(", "mFileName.c_str(), std::ios::binary") ||
		SearchFile("Huffman.h", "void Compress(", "void Decompress(", "mFileName.c_str(), std::ios_base::binary") ||
		SearchFile("Huffman.h", "void Compress(", "void Decompress(", "mFileName, std::ios_base::binary") ||
		SearchFile("Huffman.h", "void Compress(", "void Decompress(", "mFileName, std::ios_base::binary") ||
		SearchFile("Huffman.h", "void Compress(", "void Decompress(", "mFileName, std::ofstream::binary") ||
		SearchFile("Huffman.h", "void Compress(", "void Decompress(", "mFileName.c_str(), std::ofstream::binary");
...
FailResult UnitTests_Lab8::Fail_Compress_CompressedFileIsIncorrect() {
	...
	std::string checkFilename = randomFile.name;
...
#if HUFFMAN_DECOMPRESS
int UnitTests_Lab8::Battery_Decompress() {
	std::cout << "\tNext test will take a few seconds\n";
	...
	failVec.push_back(Fail_Decompress_DoesNotPassCorrectSizeForHeader);
	failVec.push_back(Fail_Decompress_UsesEofForIfstream);
	...
	int totalDeductions = UnitTestBattery("Testing Decompress", POINTS_HUFFMAN_DECOMPRESS,failVec, passVec);
	int scoreForTest = CalculatePointsForTest(POINTS_HUFFMAN_DECOMPRESS, totalDeductions);
...
FailResult UnitTests_Lab8::Fail_Decompress_DoesNotPassHeader() {
	bool passesFreqTable = SearchFile("Huffman.h", "void Decompress(", "};", ",(char*)mFrequencyTable") ||
		SearchFile("Huffman.h", "void Decompress(", "};", ",reinterpret_cast<char*>(mFrequencyTable)");
...
FailResult UnitTests_Lab8::Fail_Decompress_UsesEofForIfstream() {
	bool usesEof = SearchFile("Huffman.h", "void Decompress(", "};", ".eof()");

	FailResult result;
	result.check = usesEof == true;
	result.deduction = 3;
	result.msg = "Should not use eof for ifstream as looping condition. Valid binary data may not take up all bits of last byte";

	return result;
}

FailResult UnitTests_Lab8::Fail_Decompress_DoesNotPassCorrectSizeForHeader() {
	bool usesCorrectHeaderSize = SearchFile("Huffman.h", "void Decompress(", "};", ",1024") ||
		SearchFile("Huffman.h", "void Decompress(", "};", ",sizeof(int)*256") ||
		SearchFile("Huffman.h", "void Decompress(", "};", ",256*sizeof(int)") ||
		SearchFile("Huffman.h", "void Decompress(", "};", ",sizeof(unsigned int)*256") ||
		SearchFile("Huffman.h", "void Decompress(", "};", ",256*sizeof(unsigned int)") ||
		SearchFile("Huffman.h", "void Decompress(", "};", ",256*4") ||
		SearchFile("Huffman.h", "void Decompress(", "};", ",4*256");
...
FailResult UnitTests_Lab8::Fail_Decompress_OpensIncorrectFileForWriting() {
	bool opensInputFileWithIfstream = SearchFile("Huffman.h", "void Decompress(", "};", "_outputFile, std::ios::binary") ||
		SearchFile("Huffman.h", "void Decompress(", "};", "_outputFile, std::ios_base::binary") ||
		SearchFile("Huffman.h", "void Decompress(", "};", "_outputFile, std::ofstream::binary");
...
FailResult UnitTests_Lab8::Fail_Decompress_DecompressedFileIsTooLarge() {
	std::string uncompressedFilename = randomFile.name;
