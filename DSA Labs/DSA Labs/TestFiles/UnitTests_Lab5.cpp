...
#define POINTS_LAB5_CTOR					15
#define POINTS_LAB5_DTOR					5
#define POINTS_LAB5_CLEAR					10
#define POINTS_LAB5_INSERT_NEW				10
#define POINTS_LAB5_INSERT_EXISTING			10
#define POINTS_LAB5_FIND					5
#define POINTS_LAB5_FIND_NOT_FOUND			5
#define POINTS_LAB5_REMOVE					10
#define POINTS_LAB5_REMOVE_NOT_FOUND		3
#define POINTS_LAB5_ASSIGNMENT_OP			15
#define POINTS_LAB5_COPY_CTOR				10
...
#ifndef _GRADING
	results::WriteResults("Dictionary.h", "Dictionary.results");
#endif
	int totalScore = 0;

#if LAB5_PAIR_CTOR
	totalScore += Battery_PairConstructor();
#endif
#if LAB5_CTOR
	totalScore += Battery_DictionaryConstructor();
#endif
#if LAB5_DTOR
...
#endif
#if LAB5_INSERT_NEW
	totalScore += Battery_Insert_NewKey();
#endif
#if LAB5_INSERT_EXISTING
	totalScore += Battery_Insert_OverwriteExistingKey();
#endif
#if LAB5_FIND
	totalScore += Battery_Find_KeyExists();
#endif
#if LAB5_FIND_NOT_FOUND
	totalScore += Battery_Find_KeyDoesNotExist();
#endif
#if LAB5_REMOVE
	totalScore += Battery_Remove_KeyExists();
#endif
#if LAB5_REMOVE_NOT_FOUND
	totalScore += Battery_Remove_KeyDoesNotExist();
#endif
...
#if LAB5_PAIR_CTOR
int UnitTests_Lab5::Battery_PairConstructor() {
	FailVector failVec;
	...
	int totalDeductions = UnitTestBattery("Testing Pair constructor", POINTS_LAB5_PAIR_CTOR,failVec, passVec);
	int scoreForTest = CalculatePointsForTest(POINTS_LAB5_PAIR_CTOR, totalDeductions);
...
#if LAB5_CTOR
int UnitTests_Lab5::Battery_DictionaryConstructor() {
	FailVector failVec;
	...
	int totalDeductions = UnitTestBattery("Testing Dictionary constructor", POINTS_LAB5_CTOR,failVec, passVec);
	int scoreForTest = CalculatePointsForTest(POINTS_LAB5_CTOR, totalDeductions);
...
#if LAB5_DTOR
int UnitTests_Lab5::Battery_Destructor() {
	FailVector failVec;
	...
	int totalDeductions = UnitTestBattery("Testing destructor", POINTS_LAB5_DTOR,failVec, passVec);
	int scoreForTest = CalculatePointsForTest(POINTS_LAB5_DTOR, totalDeductions);
...
#if LAB5_CLEAR
int UnitTests_Lab5::Battery_Clear() {
	FailVector failVec;
	...
	int totalDeductions = UnitTestBattery("Testing Clear", POINTS_LAB5_CLEAR,failVec, passVec);
	int scoreForTest = CalculatePointsForTest(POINTS_LAB5_CLEAR, totalDeductions);
...
#if LAB5_INSERT_NEW
int UnitTests_Lab5::Battery_Insert_NewKey() {
	FailVector failVec;
	...
	// failVec.push_back(Fail_Insert_NewKey_NoPairAdded);
	...
	int totalDeductions = UnitTestBattery("Testing Insert (new key)", POINTS_LAB5_INSERT_NEW,failVec, passVec);
	int scoreForTest = CalculatePointsForTest(POINTS_LAB5_INSERT_NEW, totalDeductions);
...
FailResult UnitTests_Lab5::Fail_Insert_NewKey_NoPairAdded() {
	DictionaryWithValues dict = CreateDictionary();
	float newKey = RandomFloat(101, 200);
	int correctBucket = Hash(newKey);
	auto* bucket = &dict.dict->mTable[correctBucket];

	// Clear the bucket before inserting
	bucket->clear();

	dict.dict->Insert(newKey, newKey * 2);

	FailResult result;
	result.check = bucket->size() != 1;
	result.deduction = 10;
	result.msg = "Pair was not added";
...
#if LAB5_INSERT_EXISTING
int UnitTests_Lab5::Battery_Insert_OverwriteExistingKey() {
	FailVector failVec;
	...
	int totalDeductions = UnitTestBattery("Testing Insert (existing key)", POINTS_LAB5_INSERT_EXISTING,failVec, passVec);
	int scoreForTest = CalculatePointsForTest(POINTS_LAB5_INSERT_EXISTING, totalDeductions);
...
#if LAB5_FIND
int UnitTests_Lab5::Battery_Find_KeyExists() {
	FailVector failVec;
	...
	int totalDeductions = UnitTestBattery("Testing Find (existing key)", POINTS_LAB5_FIND,failVec, passVec);
	int scoreForTest = CalculatePointsForTest(POINTS_LAB5_FIND, totalDeductions);
...
#if LAB5_FIND_NOT_FOUND
int UnitTests_Lab5::Battery_Find_KeyDoesNotExist() {
	FailVector failVec;
	...
	int totalDeductions = UnitTestBattery("Testing Find (key not found)", POINTS_LAB5_FIND_NOT_FOUND,failVec, passVec);
	int scoreForTest = CalculatePointsForTest(POINTS_LAB5_FIND_NOT_FOUND, totalDeductions);
...
#if LAB5_REMOVE
int UnitTests_Lab5::Battery_Remove_KeyExists() {
	FailVector failVec;
	...
	int totalDeductions = UnitTestBattery("Testing Remove (existing key)", POINTS_LAB5_REMOVE,failVec, passVec);
	int scoreForTest = CalculatePointsForTest(POINTS_LAB5_REMOVE, totalDeductions);
...
FailResult UnitTests_Lab5::Fail_Remove_KeyExists_ReturnsFalse() {
	...
	result.deduction = 5;
	result.msg = "Returned false when key was in bucket";
...
#if LAB5_REMOVE_NOT_FOUND
int UnitTests_Lab5::Battery_Remove_KeyDoesNotExist() {
	FailVector failVec;
	...
	int totalDeductions = UnitTestBattery("Testing Remove (key not found)", POINTS_LAB5_REMOVE_NOT_FOUND,failVec, passVec);
	int scoreForTest = CalculatePointsForTest(POINTS_LAB5_REMOVE_NOT_FOUND, totalDeductions);
...
FailResult UnitTests_Lab5::Fail_Remove_KeyDoesNotExist_BucketIsCleared() {
	...
	// Making sure there is *A* Pair in the bucket (even though it may not be the correct bucket to be in)
	Pair pair(1, 2);
	pair.key = 1;
	pair.value = 2;
	dict.dict->mTable[Hash(keyToRemove)].push_back(pair);
...
#if LAB5_ASSIGNMENT_OP
int UnitTests_Lab5::Battery_AssignmentOperator() {
	FailVector failVec;
	...
	int totalDeductions = UnitTestBattery("Testing assignment operator", POINTS_LAB5_ASSIGNMENT_OP,failVec, passVec);
	int scoreForTest = CalculatePointsForTest(POINTS_LAB5_ASSIGNMENT_OP, totalDeductions);
...
FailResult UnitTests_Lab5::Fail_AssignmentOperator_NoSelfAssignmentCheck() {
	bool selfAssignmentFound = SearchFile("Dictionary.h", "Dictionary& operator=(const Dictionary& _assign)", "void Clear()", "if(this != &_assign)") ||
							   SearchFile("Dictionary.h", "Dictionary& operator=(const Dictionary& _assign)", "void Clear()", "if(this == &_assign)") ||
							   SearchFile("Dictionary.h", "Dictionary& operator=(const Dictionary& _assign)", "void Clear()", "if(&_assign != this)") ||
							   SearchFile("Dictionary.h", "Dictionary& operator=(const Dictionary& _assign)", "void Clear()", "if(&_assign == this)");
...
#if LAB5_COPY_CTOR
int UnitTests_Lab5::Battery_CopyConstructor() {
	FailVector failVec;
	...
	int totalDeductions = UnitTestBattery("Testing copy constructor", POINTS_LAB5_COPY_CTOR,failVec, passVec);
	int scoreForTest = CalculatePointsForTest(POINTS_LAB5_COPY_CTOR, totalDeductions);
